...
export default Vue.extend({
  ...
  methods: {
    GetManagerMessage: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('پیام ها')/items?$select=Id,Title,Image,Description&$filter=IsActive eq 1 and Target eq 'ستاد خبری امور حراست و حفاظت'&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
        this.ManagerMessage = response.data.value;
      });
      document.title="ستاد خبری حراست";
    },
  },
  data() {
    return {
