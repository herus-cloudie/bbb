<template src="./govahi.html"></template>
<style lang="scss" src="./govahi.scss" scoped></style>
...
export default Vue.extend({
  ...
  methods: {
         GetGovahiTitle: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('گواهی تمکن مالی(خدمات بانکی ارزی)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20eq%201'";
      axios.get(endPointUrl).then((response) => {                          
        this.GovahiTitle = response.data.value;
        document.title="گواهی تمکن مالی بانک صادرات ایران";
      });
    },

    GetGovahiItems: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('گواهی تمکن مالی(خدمات بانکی ارزی)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20gt 1&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
        this.GovahiItems = response.data.value;
      });
    },
  },
  data() {
