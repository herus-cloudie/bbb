<template src="./govahi-seporde-sarmayeghozari-am.html"></template>
<style lang="scss" src="./govahi-seporde-sarmayeghozari-am.scss" scoped></style>
...
export default Vue.extend({
  ...
  methods: {
     GetGharzolhasaneSepordehaTitle: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('گواهی سپرده مدت دار ویژه سرمایه گذاری عام(سپرده ها)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20eq%201'";
      axios.get(endPointUrl).then((response) => {
        this.GharzolhasaneSepordehaTitle = response.data.value;
        document.title="گواهی سپرده مدت دار ویژه سرمایه گذاری عام بانک صادرات ایران";
      });
    },
    GetGharzolhasaneSepordeha: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('گواهی سپرده مدت دار ویژه سرمایه گذاری عام(سپرده ها)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20gt 1&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
