<template src="./qarz-hasane-pasandaz.html"></template>
<style lang="scss" src="./qarz-hasane-pasandaz.scss" scoped></style>
...
export default Vue.extend({
  ...
  methods: {
     GetGharzolhasaneSepordehaTitle: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('قرض الحسنه پس انداز(سپرده ها)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20eq%201'";
      axios.get(endPointUrl).then((response) => {
        this.GharzolhasaneSepordehaTitle = response.data.value;
        document.title="قرض الحسنه پس انداز بانک صادرات ایران";
      });
    },
    GetGharzolhasaneSepordeha: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('قرض الحسنه پس انداز(سپرده ها)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20gt 1&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
