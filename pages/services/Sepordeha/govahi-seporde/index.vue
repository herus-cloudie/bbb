<template src="./govahi-seporde.html"></template>
<style lang="scss" src="./govahi-seporde.scss" scoped></style>
...
export default Vue.extend({
  ...
  methods: {
     GetGovahiSepordeTitle: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('گواهی سپرده خاص(سپرده ها)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20eq%201'";
      axios.get(endPointUrl).then((response) => {
        this.GovahiSepordeTitle = response.data.value;
        document.title="گواهی سپرده خاص بانک صادرات ایران";
      });
    },
    GetGovahiSeporde: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('گواهی سپرده خاص(سپرده ها)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20gt 1&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
