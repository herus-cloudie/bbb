<template src="./Vagozari-check.html"></template>
<style lang="scss" src="./Vagozari-check.scss" scoped></style>
...
export default Vue.extend({
  ...
  methods: {
      ...
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('واگذاری(خدمات بانکی ریالی)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20eq%201%20and%20Parent%20ne%20'لینک ها'";
    ...
    GetMohasebeGanjine: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('واگذاری(خدمات بانکی ریالی)')/items?$select=Id,Title,Description&$filter=IsActive%20eq%201%20and%20Order0%20gt 1%20and%20Parent%20ne%20'لینک ها'&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
    ...
    GetLink: function () {
      var endPointUrl =
        this.SiteAdrs +
        "/_api/web/lists/getbyTitle('واگذاری(خدمات بانکی ریالی)')/items?$select=Id,Title,Description,TxtLinkLast&$filter=IsActive%20eq%201%20and%20Parent%20eq%20'لینک ها'&$orderby=Order0";
      axios.get(endPointUrl).then((response) => {
